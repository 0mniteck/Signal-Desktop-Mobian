...
mkdir -p "$HOME/syft" && TMPDIR="$HOME/syft" syft / --select-catalogers debian -o spdx-json=builds/release/debian.spdx.json && TMPDIR="$HOME/syft" syft docker:signal-desktop -o spdx-json=builds/release/docker.spdx.json && rm -f -r "$HOME/syft"
...
snap install grype --classic && grype sbom:builds/release/manifest.spdx.json -o json > builds/release/manifest.grype.json && grype sbom:builds/release/debian.spdx.json -o json > builds/release/debian.grype.json && grype sbom:builds/release/docker.spdx.json -o json > builds/release/docker.grype.json
