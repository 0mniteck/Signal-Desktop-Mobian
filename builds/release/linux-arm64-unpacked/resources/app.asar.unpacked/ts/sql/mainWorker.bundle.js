...
var require_mainWorkerBindings = __commonJS({
  "ts/sql/mainWorkerBindings.ts"(exports, module2) {
...
var require_wrappers = __commonJS({
  "node_modules/better-sqlite3/lib/methods/wrappers.js"(exports) {
    ...
    exports.prepare = /* @__PURE__ */ __name(function prepare2(sql) {
    ...
    exports.close = /* @__PURE__ */ __name(function close2() {
      this[cppdb].close();
      return this;
    }, "close");
...
var require_p_map = __commonJS({
  "node_modules/p-props/node_modules/p-map/index.js"(exports, module2) {
    "use strict";
...
var require_p_props = __commonJS({
  "node_modules/p-props/index.js"(exports, module2) {
    "use strict";
    ...
    var object = async (map3, mapper, options) => {
...
var import_p_props = __toESM(require_p_props());
...
function makeEnumParser(enumToParse, defaultValue) {
  ...
  const isEnumValue = (value) => typeof value === "string" && enumValues.has(value);
...
var error = (...args) => logAtLevel(50 /* Error */, ...args);
var warn = (...args) => logAtLevel(40 /* Warn */, ...args);

// ts/util/assert.ts
function assert(condition, message) {
  if (!condition) {
    const err = new Error(message);
    if (getEnvironment() !== "production" /* Production */) {
      if (getEnvironment() === "development" /* Development */) {
        debugger;
      }
      throw err;
    }
...
// ts/types/UUID.ts
var isValidUuid = (value) => typeof value === "string" && /^[0-9A-F]{8}-[0-9A-F]{4}-4[0-9A-F]{3}-[89AB][0-9A-F]{3}-[0-9A-F]{12}$/i.test(value);
var UUID = class {
  constructor(value) {
    this.value = value;
...
var missingCaseError = (x) => new TypeError(`Unhandled case: ${reallyJsonStringify(x)}`);
...
var formatCountForLogging = (count) => {
...
function updateToSchemaVersion52(currentVersion, db, logger3) {
  ...
  logger3.info("updateToSchemaVersion52: success!");
}

// ts/sql/migrations/53-gv2-banned-members.ts
function updateToSchemaVersion53(currentVersion, db, logger3) {
  ...
  const upgradeConversation = (convo) => {
    ...
    const legacy = convo;
    const logId = `(${legacy.id}) groupv2(${legacy.groupId})`;
    if (!((_a = legacy.bannedMembersV2) == null ? void 0 : _a.length)) {
      return false;
    }
    const jsonPatch = {
    ...
    logger3.info(`updateToSchemaVersion53: Updating ${logId} with ${legacy.bannedMembersV2.length} banned members`);
    updateConversationStmt.run({
      id: legacy.id,
  ...
  db.transaction(() => {
    const allConversations = db.prepare(`
          ...
          ORDER BY id ASC;
        `).all().map(({ json }) => jsonToObject(json));
    logger3.info(`updateToSchemaVersion53: About to iterate through ${allConversations.length} conversations`);
    let updated = 0;
    for (const convo of allConversations) {
      updated += upgradeConversation(convo) ? 1 : 0;
    }
    logger3.info(`updateToSchemaVersion53: Updated ${updated} conversations`);
    db.pragma("user_version = 53");
  })();
  logger3.info("updateToSchemaVersion53: success!");
}

// ts/sql/migrations/54-unprocessed-received-at-counter.ts
function updateToSchemaVersion54(currentVersion, db, logger3) {
  ...
  logger3.info("updateToSchemaVersion54: success!");
}
...
var statementCache = /* @__PURE__ */ new WeakMap();
...
function _storyIdPredicate(storyId) {
  if (storyId === void 0) {
...
async function getUnreadByConversationAndMarkRead({
  ...
  return db.transaction(() => {
    ...
    db.prepare(`
        ...
        `).run({
      conversationId,
      jsonPatch: JSON.stringify({ readStatus: 0 /* Read */ }),
      newestUnreadAt,
      storyId: storyId || null
    });
    return rows.map((row) => {
      ...
      WHERE
        ...
        (
          ...
          (type IS 'incoming' AND (
            readStatus = ${0 /* Read */} OR
            readStatus = ${2 /* Viewed */} OR
            readStatus IS NULL
          ))
        );
      `).all();
...
async function removeAllConfiguration(mode = "Full" /* Full */) {
  const db = getInstance();
  db.transaction(() => {
    db.exec(`
      DELETE FROM identityKeys;
...
async function processGroupCallRingRequest(ringId) {
  ...
  return db.transaction(() => {
    ...
    if (wasRingPreviouslyCanceled) {
      result = 1 /* RingWasPreviouslyCanceled */;
    } else {
      ...
      if (isThereAnotherActiveRing) {
        result = 2 /* ThereIsAnotherActiveRing */;
      } else {
        result = 0 /* ShouldRing */;
      }
      db.prepare(`
